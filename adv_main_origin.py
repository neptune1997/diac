...
def stacking_models(args, stacker):
    ...
    f1 = f1_score(test_y,test_pre, average='macro')
...
def train(args):
    ...
    if args.model_name in MODEL_MAP:
        ...
        bar = tqdm(range(len(train_loader)*args.epochs), total=len(train_loader)*args.epochs)
        ...
        optimizer_grouped_parameters = [
            {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)],
             'weight_decay': 0.0},
            {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay}
        ]
        ...
        for step in bar:
            ...
            if (steps)%args.eval_steps==0:
                ...
                f1 = f1_score(dev_labels, inference_labels,labels=[0,1,2], average="macro")
...
def predict(args,is_eval=False):
    ...
    if args.model_name in MODEL_MAP:
        ...
        config = Config.from_pretrained(args.pretrained_model_path, num_labels=3)
        ...
        if is_eval:
            ...
            df['label_1'] = logits[:, 1]
            df['label_2'] = logits[:, 2]
            df[['id', 'label', 'label_pre','label_0', 'label_1', 'label_2']].to_csv(os.path.join(args.out_dir, "dev_sub.csv"),index=False)
        else:
            df['label_0'] = logits[:, 0]
            df['label_1'] = logits[:, 1]
            df['label_2'] = logits[:, 2]
            ...
            label_filename = label_filename.replace(':', '-') + ".csv"
            df[['id', 'label_0', 'label_1', 'label_2']].to_csv(os.path.join(args.out_dir, filename), index=False)
...
def main():
    ...
    ##required parameter
    parser.add_argument("--memo", default='running Li model with data using adv training ', type=str, required=False)
    parser.add_argument("--model_name", type=str, default='bert_Li', required=False)
    parser.add_argument("--data_dir", type=str, default="./data/data_set", required=False)
    parser.add_argument("--out_dir", type=str, default="./Limodel_roberta_adv", required=False)
    parser.add_argument("--pretrained_model_path", type=str, default="./chinese_roberta_small", required=False)
    parser.add_argument("--to_resume_model",type=bool,default=False, required=False)
    parser.add_argument("--resume_model_path", type=str, default="./Limodel_roberta_adv/pytorch_model.bin", required=False)
    ...
    parser.add_argument("--weight", default=[1.0, 1.0, 1.0], type=list, required=False, help='the weight for crossentropy')
    ...
    parser.add_argument("--weight_list", default=[[1.0,1.0,1.0],[2.0,1.0,1.0],[2.0,2.0,1.0],[4.0,2.0,1.0],[1.0,2.0,1.0]],type=list,required=False,help='weight list used in the ensemble mode')
    ...
    parser.add_argument("--title_seq_len", type=int, default=32, required=False)
    parser.add_argument("--content_seq_len", type=int, default=512, required=False)
