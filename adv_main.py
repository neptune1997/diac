        ...
        for _ in range(args.epochs):
            for step, data_batch in enumerate(train_loader):
                ...
                if steps>args.optimize_steps:
                    logger.info("early stopping in {} steps".format(args.optimize_steps))
                    break
...
def do_inference(model, dataloader, device):
    ...
    for step in steps:
        ...
        logits = nn.functional.softmax(logits, -1) ##let the result go through softmax
...
def main():
    ...
    ##required parameter
    parser.add_argument("--memo", default='running adv training with auged data in base', type=str, required=False)
    ...
    parser.add_argument("--out_dir", type=str, default="./adv_Limodel_bert", required=False)
    parser.add_argument("--pretrained_model_path", type=str, default="./chinese_roberta", required=False)
    parser.add_argument("--to_resume_model",type=bool,default=False, required=False)
    parser.add_argument("--resume_model_path", type=str, default="./adv_Limodel_bert/pytorch_model.bin", required=False)
    ...
    parser.add_argument("--eval_batch_size", default=32, type=int, required=False)
    ...
    parser.add_argument("--optimize_steps", type= int, default=4000, required=False)
