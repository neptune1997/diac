...
class PGD():
    """
    using pgd to do adversarial training, but the result is not as good as fgm
    """
    ...
    def attack(self, epsilon=1e-6, alpha=0.3, emb_name='embeddings', is_first_attack=False):
...
def predict(args,is_eval=False):
    ...
    if args.model_name in MODEL_MAP:
        ...
        if is_eval:
            df['label_0'] = logits[:, 0]
            df['label_1'] = logits[:, 1]
            df[['id', 'label', 'label_pre','label_0', 'label_1']].to_csv(os.path.join(args.out_dir, "dev_sub.csv"),index=False)
        else:
            df['label_0'] = logits[:, 0]
            df['label_1'] = logits[:, 1]
            filename = time.ctime().replace(' ', '-')
            ...
            with open(os.path.join(args.out_dir, label_filename),'w',encoding='utf-8') as out:
...
def main():
    ...
    ##required parameter
    parser.add_argument("--memo", default='running glue baseline Five Fold', type=str, required=False)
    parser.add_argument("--model_name", type=str, default='bert_Li', required=False)
    parser.add_argument("--data_dir", type=str, default="./data/nlp_new_train{}", required=False)
    parser.add_argument("--out_dir", type=str, default="./Five_adv2_Limodel_bert", required=False)
    ...
    parser.add_argument("--resume_model_path", type=str, default="./Five_adv2_Limodel_bert", required=False)
    ...
    parser.add_argument("--do_kfold", action="store_true", default=True, required=False)
