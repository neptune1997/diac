...
class MyRnnBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super(MyRnnBertModel, self).__init__(config)
        self.config=config
...
class RnnClassifier(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.lstm = nn.GRU(input_size=config.hidden_size, hidden_size=config.lstm_hidden_size, num_layers=config.lstm_layers, bidirectional=True,batch_first=True)
    ...
    def forward(self, text_em):
        ...
        lstm_out, hidden = self.lstm(text_em) ##the bert embedding as input
        # pooled_out = kmax_pooling(lstm_out, dim=1, k=self.config.kmax)
        # hidden = self.dropout(hidden)
        features = hidden.permute(1,0,2).reshape(text_em.size(0), -1).contiguous()
        # features = pooled_out.view(text_em.size(0), -1)
