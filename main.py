...
from models import SplitLinearModel, LinearBertModel, RnnBertModel,SPRnnBertModel,CnnBertModel,RCnnBertModel, MyRnnBertModel, MLPBertModel, PoolingBertModel,LinearBertModelSMOTE
...
import torch
class FGM():##doing the adversarial training
...
'bert_Li':(BertConfig,LinearBertModelSMOTE,BertTokenizer,base_transform),
...
def stacking_models(args, stacker): # stacking is not working
    ...
    logger.info("the f1 score over test set is: %10f"%f1) ##not working stacking
...
def train(args,Model,tokenizer,config,transform): ##
    ...
    for _ in range(args.epochs):
        for step, data_batch in enumerate(train_loader):
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm) ##grad norm according to the glue it is useful.
            optimizer.step()
            scheduler.step()
            ...
            if (steps)%args.eval_steps==0:
                ...
                f1 = f1_score(y_true=dev_labels, y_pred=inference_labels,average='macro')
                c1_f1, c2_f1 = f1_score(y_true=dev_labels, y_pred=inference_labels,average=None)
                test_f1 = f1_score(y_true=test_labels, y_pred=test_inference_labels,average='macro')
...
# def kfold_train(args,Model,tokenizer,config,transform): ##k_fold have oom error
#     k = args.folds
...
#         args.to_resume_model = False
#         train(args,Model,tokenizer,config,transform)
#         ###get the evaluation##
#         args.to_resume_model=True ##set to resume the model
#         # predict(args,Model,tokenizer,c   onfig,transform,is_eval=True) ##output the dev result
#         ###do the test set prediction###
#         # predict(args,Model,tokenizer,config,transform,is_eval=False)
...
#         args.out_dir = os.path.join(origin_out_dir,"model_{}".format(i))
#         args.resume_model_path = os.path.join(args.out_dir,"pytorch_model.bin")
#         predict(args,Model,tokenizer,con   fig,transform,is_eval=Tr("eval f1 score: %s\n"%( str(f1)))ue) ##output the dev result
#         ##do the test set prediction###
#         predict(args,Model,tokenizer,config,transform,is_eval=False)
...
def predict(args,Model,tokenizer,config,transform,is_eval=False):
    ...
    if is_eval:
        df['label_0'] = logits[:, 0]
        df['label_1'] = logits[:, 1]
        df[['id', 'label', 'label_pre','label_0', 'label_1','question1','question2']].to_csv(os.path.join(args.out_dir, "dev_sub.csv"),index=False)
    else:
        df['label_0'] = logits[:, 0]
        df['label_1'] = logits[:, 1]
        filename = time.ctime().replace(' ', '-')
        ...
        label_filename = label_filename.replace(':', '-') + ".csv"
        df[['id', 'label_0', 'label_1']].to_csv(os.path.join(args.out_dir, filename), index=False)
        df[['id', 'label_pre']].to_csv(os.path.join(args.out_dir, label_filename), index=False,header=False,sep='\t')
...
def main():
    ...
    ##required parameter
    parser.add_argument("--memo", default='running glue for smote', type=str, required=False)
    parser.add_argument("--model_name", type=str, default='bert_Li', required=False)
    parser.add_argument("--data_dir", type=str, default="./data/nlp_new_train0", required=False)
    parser.add_argument("--out_dir", type=str, default="./Limodel_bert_smote", required=False)
    ...
    parser.add_argument("--resume_model_path", type=str, default="./Limodel_bert_smote", required=False)
    ...
    parser.add_argument("--optimize_steps", type= int, default=6000, required=False)
    ...
    if args.model_name in MODEL_MAP:
        ...
        #     return
        # if args.do_kfold and not args.do_train:
        #     kfold_train(args,Model,tokenizer,config,transform)
        #     return
        if args.do_train:
            args.to_resume_model = False
            train(args,Model,tokenizer,config,transform)
        if args.do_eval:
            args.to_resume_model = True
            predict(args, Model,tokenizer,config,transform,is_eval=True)  ##output the dev set result
        if args.test:
            args.to_resume_model = True
            predict(args,Model,tokenizer,config,transform)


if __name__ == "__main__":
    os.environ['CUDA_VISIBLE_DEVICES']='2' ##set the visible devices sometimes we have set before  import torch
    main()
